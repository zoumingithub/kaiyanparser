...
daemon = False
pidfile = '/opt/log/kaiyan.portal.pid'
umask = 0
user = None
group = None
tmp_upload_dir = None
...
errorlog = '/data01/log/kaiyan.gunicorn.err'
loglevel = 'info'
accesslog = '/data01/log/kaiyan.gunicorn.access'
access_log_format = '%(h)s %(l)s %(u)s %(t)s "%(r)s" %(s)s %(b)s "%(f)s" "%(a)s" \"%({Content-Length}o)s\" REQ_LEN=%({Content-Length}i)s'
limit_request_line = 0
#import sys
#sys.path.append('..')
#logconfig = 'glog.conf'

#logger_class = 'index_generation.util.multi_process_logger.MultiProcessRotatingFileHandler'
